...
if os.path.exists(database_evi):
    print('EVI database found, using ' + database_evi)
    with open(database_evi, 'r') as the_file:
        modis_in_db = [row[0] for row in csv.reader(the_file,delimiter=',')]
else:
    print('EVI database not found, creating it for ' + database_evi)
    header_line = [str(s) for s in gauges]
    header_line.insert(0,'modis_id')
    header_line.insert(1,'date')
    header_line  = ','.join(header_line) + '\n'
    with open(database_evi,'w') as the_file:
    ...
    for file_id in files_id:
        if(file_id in modis_in_db):
            ...
            if(len(selected_files)==9):
                ...
                with open(result_evi) as csv_file:
                ...
                if(gauges == gauge_id_result):
                    ...
                    with open(database_evi,'a') as the_file:
